...
class MIMIC_IV_Datasets_text_final(Dataset):
    def __init__(self, root, pt_mode,  max_visit_num=3):
        self.root = root
        self.records_med_index = dill.load(open(
            "../data/mimic-iv/output_atc4/records_med_index_iv.pkl", 'rb'))
        self.med_his_file = dill.load(open(
            "../data/mimic-iv/output_atc4/records_med_his_iv.pkl", 'rb'))
    ...
    def __getitem__(self, index):
        ...
        cur_med = torch.zeros(246)
        cur_med[m_list] = 1
        cur_med_ml = torch.full((246,), -1)
        cur_med_ml[:len(m_list)] = torch.LongTensor(m_list)

        his_med_embeddings, his_mask = padding_visit_med(
            self.med_his_file[self.shift+index], self.max_visit_num, self.save_path)
...
class MIMIC_IV_Datasets_text_Test_final_my(Dataset):
    ...
    def __getitem__(self, index):
        ...
        his_med_embeddings, his_mask = padding_visit_med(
            self.med_his_file[self.shift+self.index_list[index]], self.max_visit_num, self.save_path)
