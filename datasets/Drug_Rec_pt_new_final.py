...
def padding_visit_med(input_med_his, max_visit_num, save_path, pt_mode):
    ...
    mask = med_his.eq(-1)
    med_mask = mask.float().masked_fill(
        mask == 1, -1e9).masked_fill(mask == 0, float(0.0))
...
class MIMIC_III_Datasets_text_final(Dataset):
    def __init__(self, root, pt_mode, max_visit_num=3):
        self.root = root
        self.med_his_file = dill.load(
            open("../data/mimic-iii/output_atc4/records_med_his_iii.pkl", 'rb'))
        self.save_path = os.path.join(self.root, pt_mode)
        self.records_med_index = dill.load(
            open("../data/mimic-iii/output_atc4/records_med_index_iii.pkl", 'rb'))
        self.shift = 0
        split_point = int(len(self.records_med_index) * 2 / 3)
    ...
    def __getitem__(self, index):
        ...
        med_index = " ".join([str(elem)
                             for elem in self.med_index_list[index]])

        m_list = self.med_index_list[index]

        cur_med = torch.zeros(217)
        cur_med[m_list] = 1

        cur_med_ml = torch.full((217,), -1)
        cur_med_ml[:len(m_list)] = torch.LongTensor(m_list)

        his_med_embeddings, his_mask = padding_visit_med(
            self.med_his_file[self.shift+index], self.max_visit_num, self.save_path, self.pt_mode)
...
class MIMIC_III_Datasets_text_Eval_final(Dataset):
    def __init__(self, root, pt_mode, max_visit_num=3):
        ...
        split_point = int(len(self.records_med_index) * 2 / 3)
        eval_len = int(len(self.records_med_index[split_point:]) / 2)
...
class MIMIC_III_Datasets_text_Test_final_my(Dataset):
    ...
    def __getitem__(self, index):
        ...
        med_index = " ".join(
            [str(elem) for elem in self.med_index_list[self.index_list[index]]])
        ...
        his_med_embeddings, his_mask = padding_visit_med(
            self.med_his_file[self.shift+self.index_list[index]], self.max_visit_num, self.save_path, self.pt_mode)
