...
class AdjAttenAgger(torch.nn.Module):
    ...
    def forward(self, main_feat, other_feat, fix_feat=None, mask=None):
        ...
        if mask is not None:
            ...
            # If a row is fully masked, disable masking for that row to avoid all -inf
            with torch.no_grad():
                all_masked = (mask == mask.min()).all(dim=-1, keepdim=True)
            safe_mask = mask.masked_fill(all_masked, 0.0)
            # Compute softmax in float32 for stability, then cast back
            Attn = (Attn + safe_mask).float()
            Attn = torch.softmax(Attn, dim=-1).to(other_feat.dtype)
        else:
            Attn = torch.softmax(Attn.float(), dim=-1).to(other_feat.dtype)
        # print(Attn[0])
        # print(mask[0])
